...
public class AuctionDao {
    ...
    private Auction mapResultSetToAuction(ResultSet resultSet) throws SQLException {
        ...
        // 🛡️ Hàm parse thông minh xử lý cả dấu cách từ SQLite
        String startTimeStr = resultSet.getString("start_time");
        LocalDateTime startTime = (startTimeStr != null) ? LocalDateTime.parse(startTimeStr.replace(" ", "T")) : LocalDateTime.now();

        String endTimeStr = resultSet.getString("end_time");
        LocalDateTime endTime = (endTimeStr != null) ? LocalDateTime.parse(endTimeStr.replace(" ", "T")) : LocalDateTime.now();
