...
public class ItemService {
    ...
    public void updateItem(Item item) throws SQLException {
        if (item == null) {
            throw new IllegalArgumentException("san pham khong tim thay de cap nhat");
        }
        itemDao.update(item);
    }

    /**
     * 🌟 HÀM ĐÃ CẬP NHẬT: Thêm tham số đầu vào để phân quyền xóa an toàn
     */
    public void deleteItem(int id, int currentUserId, String currentRole) throws SQLException {
        if (id < 0) {
            throw new IllegalArgumentException("id khong hop le");
        }
        ...
        // CHẶN BẢO VỆ: Nếu KHÔNG PHẢI ADMIN và cũng KHÔNG PHẢI chủ nhân món hàng -> Từ chối xóa!
        if (!"ADMIN".equalsIgnoreCase(currentRole) && existingItem.getSellerId() != currentUserId) {
            throw new SecurityException("Hành động bị từ chối: Chỉ ADMIN hoặc chính NGƯỜI RAO BÁN sản phẩm này mới có quyền xóa!");
        }
